 ...
 * Renders a tabular list of candidate profiles including contact info, professional records,
 * and current application status. Includes action buttons to download resumes, inspect history, 
 * modify profiles (HR only), and transition status states.
 */
export default function CandidatesTable({ candidates, user, onStatusChange, onOpenResume, onShowHistory, onEdit }) {
  return (
    <table>
      ...
      <tbody>
        {/* Render fallback empty state if no candidates exist */}
        {candidates.length === 0 ? (
          <tr>
            ...
            <tr key={c.id}>
              ...
              <td>
                <div className="actions">
                  ...
                  {/* Status transitions selector. Disabled during initial creation or schedule phases */}
                  <select
