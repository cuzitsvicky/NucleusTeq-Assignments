...
async def test_service_authenticate_user_success():
...
async def test_service_authenticate_user_invalid_credentials():
    await db.users.insert_one({
        "name": "Auth Service User",
        "email": "service_auth@nucleusteq.com",
        "password": get_password_hash("password123"),
...
async def test_service_authenticate_user_disabled():
    await db.users.insert_one({
...
async def test_service_reset_password():
