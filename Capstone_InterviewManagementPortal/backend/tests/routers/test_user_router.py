...
async def test_router_register_user(client, admin_headers, hr_headers):
    ...
    hr_resp = client.post("/api/auth/register", json=payload, headers=hr_headers)
    assert hr_resp.status_code == 403
    
    # Admin registration attempt -> 200 Success
    admin_resp = client.post("/api/auth/register", json=payload, headers=admin_headers)
...
async def test_router_get_users(client, admin_headers, hr_headers):
    ...
    admin_resp = client.get("/api/auth/users", headers=admin_headers)
    assert admin_resp.status_code == 200
...
async def test_router_get_interviewers(client, admin_headers, hr_headers, interviewer_headers):
    ...
    hr_resp = client.get("/api/auth/interviewers", headers=hr_headers)
...
async def test_router_update_and_disable_user(client, admin_headers, hr_headers):
    ...
    hr_resp = client.put(f"/api/auth/users/{user_id}", json=update_payload, headers=hr_headers)
    assert hr_resp.status_code == 403
    
    # Admin update attempt (and disabling user) -> 200 Success
    admin_resp = client.put(f"/api/auth/users/{user_id}", json=update_payload, headers=admin_headers)
    assert admin_resp.status_code == 200
...
async def test_router_update_user_self_block(client, admin_headers):
    ...
    resp = client.put(f"/api/auth/users/{admin_id}", json=payload, headers=admin_headers)
