...
async def schedule_interview(interview_data: dict, current_user_email: str):
    ...
    await candidate_repo.update_candidate_status(
        interview_data["candidate_id"], "INTERVIEW_SCHEDULED"
    )
    await candidate_repo.add_status_history(
        interview_data["candidate_id"], "INTERVIEW_SCHEDULED", current_user_email
    )
    return interview_id
...
async def submit_feedback(interview_id: str, feedback_data: dict, current_user_email: str):
    ...
    await candidate_repo.add_status_history(
        candidate_id, "INTERVIEW_COMPLETED", current_user_email
    )
    logger.info(
