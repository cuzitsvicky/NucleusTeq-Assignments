...
async def download_resume_for_user(
    candidate_id: str, role: str, email: str
) -> tuple[bytes, str]:
...
async def get_candidates_for_user(
    ...
    if role == UserRole.INTERVIEWER:
        candidate_ids = await interview_repo.get_candidate_ids_by_interviewer_email(
            email_user
        )
...
async def update_candidate(
    candidate_id: str, candidate_data: dict, current_user_email: str
):
    """
