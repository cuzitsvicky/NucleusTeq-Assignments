...
from .auth import get_current_user, check_password_reset
...
async def get_users(page: int = 1, current_user: dict = Depends(check_password_reset)):
    """
    ...
    return await user_service.get_users(page=page)


@router.get("/interviewers", response_model=List[UserResponse])
async def get_interviewers(current_user: dict = Depends(check_password_reset)):
    """
