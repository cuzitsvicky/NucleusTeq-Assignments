...
async def get_jobs(page: int = 1, current_user: dict = Depends(check_password_reset)):
    if page < 1:
...
async def update_job(
    job_id: str, job: JobCreateRequest, current_user: dict = Depends(check_password_reset)
):
    if current_user["role"] not in ["Admin", "HR"]:
        raise ForbiddenException(detail="Not authorized")
    if not ObjectId.is_valid(job_id):
        raise BadRequestException("Invalid job ID format")
    existing = await job_service.get_job_by_id(job_id)
    if not existing:
        raise NotFoundException("Job not found")
