...
async def get_all_candidates(page: int = 1, limit: int = 10, name: str = "", email: str = "", status: str = "", applied_job_id: str = ""):
...
async def get_candidate_by_email_or_mobile_exclude(email: str,mobile: str,exclude_id: str):
...
async def get_status_history(candidate_id: str, page: int = 1, limit: int = 10):
    ...
    return await paginate_collection(db.status_history, {"candidate_id": candidate_id}, page, limit, sort=("timestamp", -1))
...
async def get_candidates_by_ids(candidate_ids: list, page: int = 1, limit: int = 10):
    ...
    return await paginate_collection(db.candidates, {"_id": {"$in": object_ids}}, page, limit, sort=("_id", -1))
